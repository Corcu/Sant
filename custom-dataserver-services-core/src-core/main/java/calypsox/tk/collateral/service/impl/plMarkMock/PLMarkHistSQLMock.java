 ...
 */
public class PLMarkHistSQLMock extends AuthorizableSQL<PLMark> {

    private static PLMarkLoaderMock _plMarkLoader = new PLMarkLoaderMock();
    private static PLMarkValueLoaderMock _markValueLoader = new PLMarkValueLoaderMock();

    public PLMarkHistSQLMock() {
    }

    public static PLMark get(final long markId) throws PersistenceException {
        final PLMark pLMark = (PLMark) _plMarkLoader.loadFromDB(PLMarkLoaderMock.SELECT_BY_ID,
                new Object[]{markId});
    ...
    public static PLMark get(final long tradeId, final String subId, final String pricingEnvName,
        ...
        if ((subId == null) || ((isOracle()) && (subId.equals("")))) {
            ...
            pLMark = (PLMark) _plMarkLoader.loadFromDB(PLMarkLoaderMock.SELECT_BY_TRADE_ENV_VALDATE_WITH_NULL_SUBID,
                    values);
        } else {
            values = new Object[]{tradeId, subId, pricingEnvName, valuationDate};
            pLMark = (PLMark) _plMarkLoader.loadFromDB(PLMarkLoaderMock.SELECT_BY_TRADE_ENV_VALDATE, values);
        }
        loadPLMarkValues(pLMark);
    ...
    private static PLMark get(final int markId, final String markName, final String currency, final String markType)
        ...
        final PLMark pLMark = (PLMark) _plMarkLoader.loadFromDB(
                "SELECT mark_id, trade_id, pricing_env_name, valuation_date, position_or_trade, position_or_trade_version, entered_datetime, update_datetime, version_num, entered_user, sub_id, book_id, position_time, market_time, comments, status FROM PL_MARK_HIST where mark_id = ?",
    ...
    public static Set<PLMark> load(final Map<Integer, HashSet<String>> tradeMap, final String pricingEnvName,
        ...
        for (int i = 0; i < listStringsOfIntegers.size(); i++) {
            final String query = "SELECT mark_id, trade_id, pricing_env_name, valuation_date, position_or_trade, position_or_trade_version, entered_datetime, update_datetime, version_num, entered_user, sub_id, book_id, position_time, market_time, comments, status FROM PL_MARK_HIST  WHERE pricing_env_name = "
    ...
    private static void loadPLMarkValues(final PLMark pLMark) throws PersistenceException {
        ...
        final String query = "SELECT mark_id, mark_name, mark_value, adj_value, currency, display_class, display_digits, is_adjusted, adj_type, adj_comment, mark_type FROM pl_mark_value_hist  WHERE mark_id = "
                + pLMark.getId();
    ...
    private static void loadPLMarkValues(final Set<Integer> markIdSet, final Map<Integer, PLMark> pLMarkMap)
        ...
        for (int i = 0; i < listMarkId.size(); i++) {
            final String query = "SELECT mark_id, mark_name, mark_value, adj_value, currency, display_class, display_digits, is_adjusted, adj_type, adj_comment FROM pl_mark_value_hist  WHERE mark_id IN ("
                    + listMarkId.get(i) + ")";
    ...
    private static void loadPLMarkValue(final PLMark pLMark, final String markName, final String currency,
        ...
        final PLMarkValue listResultDB = (PLMarkValue) _markValueLoader.loadFromDB(PLMarkValueLoaderMock.SELECT_BY_VALUES,
                values);
    ...
    public static Collection<PLMark> get(final String pricingEnvName, final JDate valuationDate)
            throws PersistenceException {
        final List<PLMark> listResultDB = _plMarkLoader.listFromDB(PLMarkLoaderMock.SELECT_BY_ENV_AND_DATE,
    ...
    public static int count(final String clause) throws PersistenceException {
        String query = "SELECT count(*) FROM PL_MARK_HIST";
    ...
    public static Collection<PLMark> get(final String clause) throws PersistenceException {
        ...
        String query = "SELECT mark_id, trade_id, pricing_env_name, valuation_date, position_or_trade, position_or_trade_version, entered_datetime, update_datetime, version_num, entered_user, sub_id, book_id, position_time, market_time, comments, status FROM PL_MARK_HIST ";
        if (!Util.isEmpty(clause)) {
            query = query + " WHERE " + clause;
        }
    ...
    public static Vector<Integer> getPLMarkIds(final Connection connection, final String clause)
        ...
        try {
            ...
            stringBuffer.append("SELECT mark_id FROM ").append(PLMarkLoaderMock.TABLE_NAME_HISTORY);
    ...
    public static PLMark getLatest(final long tradeId, final String subId, final String pricingEnvName,
        ...
        if ((subId == null) || ((isOracle()) && (subId.equals("")))) {
            ...
            i = _plMarkLoader.getSingleIntEntry(PLMarkLoaderMock.SELECT_LATEST_BY_TRADE_ENV_VALDATE_WITH_NULL_SUBID, values,
                    0);
        } else {
            values = new Object[]{tradeId, subId, pricingEnvName, valuationDate, markName, markType,
                    currency};
            i = _plMarkLoader.getSingleIntEntry(PLMarkLoaderMock.SELECT_LATEST_BY_TRADE_ENV_VALDATE, values, 0);
        }
        if (i > 0) {
        ...
        if (!Util.isEmpty(pricingEnvName)) {
            ...
            if ((subId == null) || ((isOracle()) && (subId.equals("")))) {
                ...
                i = _plMarkLoader.getSingleIntEntry(
                        PLMarkLoaderMock.SELECT_LATEST_MARK_BY_TRADE_ENV_VALDATE_WITH_NULL_SUBID, values, 0);
            } else {
                values = new Object[]{tradeId, subId, pricingEnvName, valuationDate};
                i = _plMarkLoader.getSingleIntEntry(PLMarkLoaderMock.SELECT_LATEST_MARK_BY_TRADE_ENV_VALDATE, values, 0);
            }
            if (i > 0) {
                pLMark = get(i);
            }
            ...
            if ((subId == null) || ((isOracle()) && (subId.equals("")))) {
                ...
                listResultDB = _plMarkLoader
                        .listFromDB(PLMarkLoaderMock.SELECT_LATEST_MARK_BY_TRADE_VALDATE_WITH_NULL_SUBID, values);
                loadPLMarkValues(listResultDB);
            }
    ...
    public static Collection<PLMark> getMaxDatedPLMarks(final String clause, final boolean existsSubId)
        ...
        if (!existsSubId) {
            ...
            final StringBuffer stringBuffer = new StringBuffer(PLMarkLoaderMock.SELECT_MAX_DATED_MARKS_NULL_SUBID);
            if (!Util.isEmpty(clause)) {
    ...
    public static boolean bookHasUnlockedMarks(final int bookId, final String pricingEnvName, final JDate valuationDate)
        ...
        try {
            ...
            preparedStatement = ioSQL.newPreparedStatement(connection,
                    "SELECT count(*) FROM PL_MARK_HIST WHERE book_id = ? AND pricing_env_name = ? AND valuation_date = ? AND status != ?");
            int i = 1;
    ...
    public static int countPLMarks(final String clause, final Connection connection) throws PersistenceException {
        ...
        try {
            statement = ioSQL.newStatement(connection);
            final StringBuffer stringBuffer = new StringBuffer();
            stringBuffer.append("SELECT COUNT(*) FROM ").append(PLMarkLoaderMock.TABLE_NAME_HISTORY);
            if ((clause != null) && (clause.trim().length() > 0)) {
                stringBuffer.append(" WHERE ").append(clause);
            }
            jResultSet = new JResultSet(statement.executeQuery(stringBuffer.toString()));
    ...
    public static String getPLMarkIds(final String clause, final Connection connection) throws PersistenceException {
        ...
        try {
            ...
            sBufferQuery.append("SELECT mark_id FROM ").append(PLMarkLoaderMock.TABLE_NAME_HISTORY);
            if ((clause != null) && (clause.trim().length() > 0)) {
