...
public class MICExtracontableCARFBuilder extends MICExtracontableTradeBuilder {


    CA ca = null;
    Bond underlyingProductBond = null;


    public MICExtracontableCARFBuilder(Trade trade) {
        super(trade);
        Product tradeProduct = Optional.ofNullable(trade).map(Trade::getProduct).orElse(null);
        if (tradeProduct instanceof CA && trade.getProduct().getUnderlyingProduct() instanceof Bond) {
            ca = (CA) tradeProduct;
            underlyingProductBond = (Bond) ca.getUnderlyingProduct();
        }
    }
    ...
    private Product getBondReference() {
        ...
        if (Optional.ofNullable(underlyingProductBond).map(Bond::getProduct)
                .orElse(null) instanceof Bond) {
            product = underlyingProductBond.getProduct();
        }
        return product;
    }
