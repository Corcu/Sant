...
public class SantCABondRecodDaysStaticDataFilter implements StaticDataFilterInterface {
	...
	public Vector getTypeDomain(String attributeName) {
		...
		if (attributeName.equals(CA_BOND_RD)) {
		...
		if (trade != null) {
			...
			if (prod instanceof CA) {
				...
				if (p instanceof Bond) {
					return (double) ((Bond) p).getRecordDays();
				}
			}
		}
		return 0d;
