...
public class MT542CRESTSWIFTFormatter extends MT542SWIFTFormatter {

    /** CRST for storing the value CRST used by tag 22 & tag 95 */
    ...
    public String parseCREST_PRIR(BOMessage message, Trade trade, LEContact sender, LEContact rec, Vector transferRules, BOTransfer transfer, DSConnection dsCon) {
        return super.parsePRIR(message, trade, sender, rec, transferRules, transfer, dsCon) == null ? "" : ":PRIR//0010";
    }
    ...
    public String parseCREST_NARRATIVE(BOMessage message, Trade trade, LEContact sender, LEContact rec, Vector transferRules, BOTransfer transfer, DSConnection dsCon) {
        ...
        return "/HSMT/ZHDR" + SwiftMessage.END_OF_LINE +
                ...
                ":79:/INUM/542" + SwiftMessage.END_OF_LINE +
    ...
    public String parseCREST_SETTLEMENT_PARTY(BOMessage message, Trade trade, LEContact sender, LEContact rec, Vector transferRules, BOTransfer transfer, String format, DSConnection con) {
        ...
        if (TAG_95.startsWith(":PSET//UNKNOWN")){
            ...
            return ":PSET//" + message.getSenderAddressCode();
        } else if (TAG_95.startsWith(":BUYR//") || TAG_95.startsWith(":DEAG//") || TAG_95.startsWith(":REAG//")) {
            return TAG_95.substring(0,6) + CRST + "/" + mAccount;
        } else {
            TAG_95 = ":SELL/";
            return ":SELL/" + CRST + "/" + bAccount;
        }
    }

    public String parseCREST_SELL(BOMessage message, Trade trade, LEContact sender, LEContact rec, Vector transferRules, BOTransfer transfer, DSConnection dsCon) {
        if (transfer == null || TAG_95.startsWith(":SELL/")){
            return "";
        } else {
            int id = transfer.getInternalSettleDeliveryId();
            SettleDeliveryInstruction si = BOCache.getSettleDeliveryInstruction(dsCon, id);
    ...
    public boolean isNotTagSELL(BOMessage message, Trade trade, LEContact sender, LEContact rec, Vector transferRules, BOTransfer transfer, DSConnection con) throws MessageFormatException {
        return !TAG_95.startsWith(":SELL/");
    }
