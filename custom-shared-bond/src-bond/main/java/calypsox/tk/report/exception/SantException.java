...
public class SantException extends Exception {
    ...
    private final SantExceptionType santExceptionType;
     ...
     * @param source
     *            component which raised the exception, should be the
     ...
     *            product Id
     * @param objectId
     *            if the exception can be related with another object (like a
     *            BOTransfer) this field indicate its id
     * @param objectClassName
     *            this is the class' short name of the object indicated in
     *            objectId (i.e. "BOTransfer") or null if objectId == 0
     * @param Book
     *            Id book id
     */
    public SantException(final SantExceptionType exceptionType,
        ...
        this.task = createTask(exceptionType, null, source, comment, tradeId,
                productId, objectId, objectClassName, bookId);
    }
    ...
    public SantException(final SantExceptionType exceptionType,
        ...
        this.santExceptionType = exceptionType;
        this.task = createTask(exceptionType, null, source, comment, tradeId,
                0, objectId, objectClassName, bookId);
    }
    ...
    public SantException(final SantExceptionType exceptionType,
        ...
        super(comment);
        this.santExceptionType = exceptionType;
        this.task = createTask(exceptionType, eventClass, source, comment,
                tradeId, productId, objectId, objectClassName, bookId);
    }
     ...
     * @return
     */
    Task createTask(final SantExceptionType exceptionType,
            final String eventClass, final String source, final String comment,
            final long tradeId, final int productId, final long objectId,
            final String objectClassName, final int bookId) {
        final Task newTask = new Task();

        newTask.setTradeId(tradeId);
        ...
        if (exceptionType.equals(SantExceptionType.STOP_PAYMENT_THRESHOLD)) {
    ...
    private static String findInternalRef(final Task task) {
        final List list = BOCache
                .getTaskInternalRefs(DSConnection.getDefault());
        ...
        for (int j = 0; j < list.size(); j++) {
            ...
            if (!taskIntRef.getEventClass().equals("Transfer")
                    || (taskIntRef.getLevel() <= i)) {
                continue;
            }
            if (taskIntRef.getSDFilter().equals("NONE")) {
                i = taskIntRef.getLevel();
                internalRef = (new StringBuilder()).append(internalRef)
                        .append(taskIntRef.getValue()).toString();
                continue;
            }
            final StaticDataFilter staticdatafilter = BOCache
                    .getStaticDataFilter(DSConnection.getDefault(),
                            taskIntRef.getSDFilter());
            if ((staticdatafilter != null)
                    && staticdatafilter.accept(null, null, null, task)) {
                i = taskIntRef.getLevel();
                internalRef = (new StringBuilder()).append(internalRef)
                        .append(taskIntRef.getValue()).toString();
            }
     ...
     */
    public SantExceptionType getSantExceptionType() {
