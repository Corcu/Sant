 ...
 *
 */
public class SantDerivativeTradesLoader extends Thread {
    ...
    private List<String> newTradesReferencesList = new ArrayList<String>();
    private Map<String, SantTradeContainer> tradesMap;


    public SantDerivativeTradesLoader(List<String> boReferenceList){

        this.tradesReferencesList = boReferenceList;
        this.tradesMap = new HashMap<String, SantTradeContainer>(SQL_IN_ITEM_COUNT+1);
    }

    public SantDerivativeTradesLoader(String[] boReferenceArray){
    ...
    public void run(){
        ...
        for(String boReference : this.tradesReferencesList){
           // this.newTradesReferencesList.add(boReference);
            this.newTradesReferencesList.add(CONUK+boReference);
        }

        if ((this.tradesReferencesList.size()+ this.newTradesReferencesList.size()) <= SQL_IN_ITEM_COUNT){
            String processingOrg = CollateralUtilities.getDomainValues(CONSUMER_UK_CPTY).get(0);
            try {
                LegalEntity poLE = null;
                if(!StringUtil.isNullOrEmpty(processingOrg)) {
                    poLE = DSConnection.getDefault().getRemoteReferenceData().getLegalEntity(processingOrg);
                }
                final String fromQuery = "trade, trade_keyword kwd1, book bo";
                ...
                if(this.newTradesReferencesList.size()>0 && null!=poLE) {
                    int processingOrgId = poLE.getId();
                    whereQuery.append(" or (kwd1.keyword_name='BO_REFERENCE' and kwd1.keyword_value IN ");
                    whereQuery.append(Util.collectionToSQLString(newTradesReferencesList));
                    whereQuery.append(" and bo.book_id=trade.book_id and bo.legal_entity_id=");
                    whereQuery.append(processingOrgId);
                    whereQuery.append("))");
                } else{
                    whereQuery.append(")");
                }
            ...
            Log.error(SantDerivativeTradesLoader.class, "Not possible to recover more than "+ SQL_IN_ITEM_COUNT +" trades per query. It has been requested " + this.newTradesReferencesList.size()+ " items");
        }
    }
