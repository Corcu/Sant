...
/**
 * Release only the reserved substitution allocations, if any
 * 
 * @author aela
 * 
 */
public class SantReleaseSubsitutionsCollateralRule extends
	...
	protected WorkflowResult apply(TaskWorkflowConfig taskConfig,
		...
		List<MarginCallAllocation> removedMarginAllocs = SantCollateralOptimUtil
				.removeMarginAllocations(entry);
		// apply the core release process
		wr = super.apply(taskConfig, entry, dsCon);
		// add back the Margin allocation
		if (!Util.isEmpty(removedMarginAllocs)) {
			for (MarginCallAllocation alloc : removedMarginAllocs) {
