package calypsox.tk.anacredit.util.positionkeeper;
...
import com.calypso.tk.report.inventoryposition.InventoryAggregationFilterDescriptor;
...
/*	class FXCache {
	...
	void loadAndSetOpenPositions(PLPositionArray plpos, TradeFilter tf) throws Exception {
		...
		if (openPos == null || openPos.size() == 0)
			return;
		Hashtable<PLPositionKey, PLPosition> h = new Hashtable<PLPositionKey, PLPosition>();
		int size = plpos.size();
		for (int i = 0; i < size; i++) {
	 ...
	 * @param report
	 */
	public synchronized ArrayList<HashMap<String, Object>> getPositionKeeperContent(Report report, Product product) {
		if (report != null) {
			updateFromReport(report, product);
		}
		initDomains();
	 ...
	 * @param report
	 */
	public synchronized void updateFromReport(Report report, Product product) {

		setValuationDateTime(report.getValuationDatetime());
		setPricingEnv(report.getPricingEnv());
		String tradeFilter = "EOD_EQTY_VAL_REAL";
		setTradeFilter(tradeFilter);
		String aggregation = "LegalEntity";
		setAggregation(aggregation);
		setZeroPosition(1);
		setProduct(product);
		setIncludeFees(Boolean.FALSE);
		setPosBySettleDate(Boolean.FALSE);

		PosAggregationFilterDescriptor descriptor = new LiquidationAggregationFilterDescriptor();
		setPosAggregationFilterDescriptor(descriptor);
