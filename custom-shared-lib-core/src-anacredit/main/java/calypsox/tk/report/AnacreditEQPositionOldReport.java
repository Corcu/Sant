...
public class AnacreditEQPositionOldReport extends Report implements  IAnacreditReport {
    ...
    private void enrichBuysAndSells(List<ReportRow> reportRows) {
        ...
        reportRows.stream().forEach(reportRow -> {
            ...
            if (identifier.isEQ() || identifier.isEQNOT()) {
                Double totalBuy = getTotalBuySell(identifier, true,false);
                Double totalSell = getTotalBuySell(identifier, false,false);
                Double monthBuy = getTotalBuySell(identifier, true,true);
                Double monthSell = getTotalBuySell(identifier, false,true);
    ...
    private Double getTotalBuySell(EquityTypeIdentifier identifier, boolean isBUY, boolean lasMonth) {
    ...
    private String getBetweenDates(){
        ...
        dates = " and trunc(t.value_date) between " + Util.date2SQLString(firstDayOfMonth.getTime()) + " AND " + Util.date2SQLString(lastDayOfMonth.getTime());
