 ...
 * @version 1.1
 */
public class SantEmirCLMReport extends MarginCallDetailEntryReport {
     ...
     */
    private List<String> AUDIT_STATUSES = Arrays.asList("CANCELED", "TERMINATED", "MATURED", "CHECKED");
    ...
    public ReportOutput load(final Vector errorMsgs) {
        ...
        // get audit values
        String where = "entity_field_name in('MODKEY#MC_CONTRACT_NUMBER','_status') AND entity_class_name='Trade' AND TRUNC(modif_date) = "
                + Util.date2SQLString(valDatetime);

        HashMap<Integer, FieldModification> auditContractChangeTradesIds = new HashMap<Integer, FieldModification>();
        HashMap<Integer, FieldModification> auditStatusChangeTradesIds = new HashMap<Integer, FieldModification>();

        try {
            Vector<AuditValue> autitValues = DSConnection.getDefault()
                    .getRemoteTrade()
                    .getAudit(where, "version_num DESC, modif_date ASC", null);

            for (AuditValue av : autitValues) {
                av.getEntityId();
                ...
                if ("_status".equals(fm.getName())
                        && AUDIT_STATUSES.contains(fm.getNewValue()) && "VERIFIED".equals(fm.getOldValue())) {
            ...
            where = "PRODUCT_DESC.PRODUCT_SUB_TYPE in " + SantEmirUtil.getEmirSubProductTypesFromDV()
            ...
            TradeArray allTradesArray = DSConnection.getDefault()
                    .getRemoteTrade().getTrades(null, where, null, null);
            ...
            for (int i = 0; i < allTradesArray.size(); i++) {
                ...
                if ((SantEmirUtil.isStatusAccepted(allTradesArray.get(i))) && !(SantEmirUtil.isInternal(allTradesArray.get(i)))
                        && (SantEmirUtil.hasUtiOrBoReference(allTradesArray.get(i)))
                        && !(allTradeIds.contains(allTradesArray.get(i).getLongId()))) {

                    allTradeIds.add(allTradesArray.get(i).getLongId());
                    allTrades.put(allTradesArray.get(i).getLongId(),
                            allTradesArray.get(i));
            ...
            if (reportRows != null && reportRows.length > 0) {
                for (ReportRow row : reportRows) {
                    ...
                    MarginCallDetailEntryDTO detailEntry = (MarginCallDetailEntryDTO) row
                            .getProperty("Default");
                    ...
                    CollateralConfig col_conf = CacheCollateralClient.getCollateralConfig(DSConnection.getDefault(),
                            detailEntry.getMarginCallConfigId());

                    if ((detailEntry != null) && (col_conf != null) && (isTradeReportable(col_conf))) {
                        ...
                        if (auditStatusChangeTradesIds
                                .containsKey(detailEntry.getTradeId())) {
                            row.setProperty("StatusChange",
                                    auditStatusChangeTradesIds
                                            .get(detailEntry.getTradeId()));
                        }
                        ...
                        if (!contractIds.contains(detailEntry
                                .getMarginCallConfigId())) {
                            contractIds.add(detailEntry.getMarginCallConfigId());
                        }

                        if (auditContractChangeTradesIds
                                .containsKey(detailEntry.getTradeId())) {
                            ReportRow newRow = null;
                            newSantEMIRCLM = new SantEmirCLM(detailEntry);
                            newSantEMIRCLM.setId(-detailEntry.getId());
                            newRow = (ReportRow) row.clone();
                            newRow.setProperty("Default", newSantEMIRCLM);
                            newRow.setProperty("ContratChange",
                                    auditContractChangeTradesIds
                                            .get(detailEntry.getTradeId()));
            ...
            for (ReportRow row : newReportRows) {
                ...
                SantEmirCLM santEMIRCLM = (SantEmirCLM) row
                        .getProperty("Default");

                long tradeId = santEMIRCLM.getMarginCallDetailEntyDTO()
                        .getTradeId();
                if (allTrades.get(tradeId) != null) {
                    santEMIRCLM.setTrade(allTrades.get(tradeId));
                }
                row.setProperty(
                        "SantEmirCLMReportItem",
                        createItem(santEMIRCLM.getTrade(), santEMIRCLM
                                        .getMarginCallDetailEntyDTO(),
                                CacheCollateralClient.getCollateralConfig(
                                        DSConnection.getDefault(), santEMIRCLM
                                                .getMarginCallDetailEntyDTO()
                                                .getMarginCallConfigId()),
                                valDatetime));
            }

            dro.setRows(newReportRows.toArray(new ReportRow[newReportRows
                    .size()]));
     ...
     * @param trade trade
     */
    private SantEmirCLMReportItem createItem(final Trade trade,
                                             final MarginCallDetailEntryDTO mcd,
                                             CollateralConfig colaterallConfig, JDatetime valDatetime) {
        SantEmirCLMReportLogic emirLogic = null;
        SantEmirCLMReportItem item = new SantEmirCLMReportItem(trade);

        try {
            emirLogic = new SantEmirCLMReportLogic(trade, mcd, valDatetime,
                    colaterallConfig, DSConnection.getDefault()
                    .getUserDefaults().getHolidays());
