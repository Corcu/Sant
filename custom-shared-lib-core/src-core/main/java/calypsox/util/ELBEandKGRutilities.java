 ...
 *
 * @author David Porras Martínez
 */
public class ELBEandKGRutilities {

    private static final String FITCH = "Fitch";
    private static final String FITCH_NACIONAL = "FITCH_NACIONAL";
    ...
    public static Vector<CreditRating> getCreditRatingsForLE(CollateralConfig marginCall, Vector<String> agencies,
                                                             int leID, JDate date) {
        ...
        try {
            mccRatingConfig = CollateralUtilities.getMCRatingConfiguration(marginCall.getPoRatingsConfigId());
        } catch (Exception e) {
        ...
        if (Util.isEmpty(agencies)) {
            // moody
            CreditRating ratingMoody = ELBEandKGRutilities.getMoody(ratingType, leID, date);
            if (ratingMoody != null) {
                ownerCreditRatings.add(ratingMoody);
            }
            // sp
            CreditRating ratingSP = ELBEandKGRutilities.getSnP(ratingType, leID, date);
            if (ratingSP != null) {
                ownerCreditRatings.add(ratingSP);
            }
            // fitch
            CreditRating ratingFitch = ELBEandKGRutilities.getFitch(ratingType, leID, date);
            if (ratingFitch != null) {
    ...
    // Get creditRatings for given agencies an date
    public static Vector<CreditRating> getCreditRatingsForLE(CollateralConfig marginCall, Vector<String> agencies,
                                                             int leID, JDate date, String ratingType) {
    ...
    // Get creditRatings for given agencies an date 2
    public static Vector<CreditRating> getCreditRatingsForLEs(CollateralConfig marginCall, Vector<String> agencies,
                                                             int leID, JDate date, String ratingType) {
        ...
        if (Util.isEmpty(agencies)) {
            ...
            // fitch_nacional
            CreditRating ratingFitchNacional = ELBEandKGRutilities.getFitchNacional(ratingType, leID, date);
            if (ratingFitch != null) {
                ownerCreditRatings.add(ratingFitchNacional);
            }
        } else {
            for (String agency : agencies) {
                if (agency.equals(MOODY)) {
                ...
                if (agency.equals(FITCH_NACIONAL)) {
                    CreditRating ratingFitchNacional = ELBEandKGRutilities.getFitchNacional(ratingType, leID, date);
                    if (ratingFitchNacional != null) {
                        ownerCreditRatings.add(ratingFitchNacional);
                    }
                }
            }
    ...
    public static GlobalRating getAgencyGlobalRating(GlobalRatingConfiguration globalRatingConfig, String agency) {
        List<GlobalRating> globalRatings = globalRatingConfig.getGlobalRating(CreditRating.CURRENT, agency,
                CreditRating.ANY);
    ...
    public static CreditRating getMoody(String ratingType, int leId, JDate valDate) {
    ...
    public static CreditRating getFitch(String ratingType, int leId, JDate valDate) {
        return getLEAgencyCreditRating(ratingType, leId, FITCH, valDate);
    }

    // Get Fitch rating for a legalEntity
    public static CreditRating getFitchNacional(String ratingType, int leId, JDate valDate) {
        return getLEAgencyCreditRating(ratingType, leId, FITCH_NACIONAL, valDate);
    }


    // Get agency rating for a specified agency and legalEntity
    public static CreditRating getLEAgencyCreditRating(String ratingType, final int leId, final String agency,
                                                       JDate valDate) {
    ...
    public static int getAgencyRatingPriority(GlobalRating globalRating, String ratingValue) {
    ...
    public static String getAgencyRatingValue(GlobalRating globalRating, int priority) {
        GlobalRatingValue globalRatingValue = globalRating.getGlobalRatingValue(priority);
    ...
    public static MarginCallCreditRating getMccrt(Vector<CreditRating> creditRatings, int ratingsConfigId,
                                                  String ratingDirection, JDate date) {

        if (!Util.isEmpty(ratingDirection)) {

            Vector<MarginCallCreditRating> mccRatings = getMarginCallCreditRatings(creditRatings, ratingsConfigId,
                    date);
            //return MarginCallCreditRating.filterPriority(mccRatings, ratingDirection);
    ...
    public static Vector<MarginCallCreditRating> getMarginCallCreditRatings(Vector<CreditRating> ratings,
                                                                            int ratingConfigId, JDate valDate) {
        ...
        try {
            ...
            Log.error(ELBEandKGRutilities.class, e); //sonar
        }
        return mccRatings;
    }
    ...
    public static boolean isThresholdDependingOn(CollateralConfig marginCall, String type) {
    ...
    public static boolean isCptyThresholdDependingOn(CollateralConfig marginCall, String type) {
    ...
    public static boolean isIADependingOnRating(CollateralConfig marginCall) {
        if (!Util.isEmpty(marginCall.getPoIARatingDirection()) && !marginCall.getPoIARatingDirection().equals(NONE)) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isCptyIADependingOnRating(CollateralConfig marginCall) {
        if (!Util.isEmpty(marginCall.getLeIARatingDirection()) && !marginCall.getLeIARatingDirection().equals(NONE)) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isMTADependingOn(CollateralConfig marginCall, String type) {
    ...
    public static boolean isCptyMTADependingOn(CollateralConfig marginCall, String type) {
    ...
    public static boolean isOwnerKGRcontractDependingOnRating(CollateralConfig marginCall) {
        return isThresholdDependingOn(marginCall, CollateralConfig.GLOBAL_RATING) || isIADependingOnRating(marginCall)
                || isMTADependingOn(marginCall, CollateralConfig.GLOBAL_RATING);
    }

    // Check if KGR contract cpty depends on rating
    public static boolean isCptyKGRcontractDependingOnRating(CollateralConfig marginCall) {
        return isCptyThresholdDependingOn(marginCall, CollateralConfig.GLOBAL_RATING)
                || isCptyIADependingOnRating(marginCall)
                || isCptyMTADependingOn(marginCall, CollateralConfig.GLOBAL_RATING);
    }

    // Check if KGR contract depends on rating
    public static boolean isKGRcontractDependingOnRating(CollateralConfig marginCall) {
        return isOwnerKGRcontractDependingOnRating(marginCall) || isCptyKGRcontractDependingOnRating(marginCall);
    }

    // Check if ELBE contract owner depends on rating
    public static boolean isOwnerELBEcontractDependingOnRating(CollateralConfig marginCall) {
        return isThresholdDependingOn(marginCall, CollateralConfig.GLOBAL_RATING) || isIADependingOnRating(marginCall)
                || isMTADependingOn(marginCall, CollateralConfig.GLOBAL_RATING);
    }

    // Cheak if ELBE contract cpty depends on rating
    public static boolean isCptyELBEcontractDependingOnRating(CollateralConfig marginCall) {
        return isCptyIADependingOnRating(marginCall);
    }

    // Check if ELBE contract depends on rating
    public static boolean isELBEcontractDependingOnRating(CollateralConfig marginCall) {
        return isOwnerELBEcontractDependingOnRating(marginCall) || isCptyELBEcontractDependingOnRating(marginCall);
    }

    // *** CollateralManager Contract Info *** //

    // Get total exposure from contract and date
    public static double getMccTotalExposure(final CollateralConfig marginCall, JDate date) {
    ...
    public static double getMccTotalNominal(final CollateralConfig marginCall, JDate date) {
        ...
        try {
            ...
            if (!Util.isEmpty(entries)) {
                List<MarginCallDetailEntryDTO> detailEntries = entries.get(0).getDetailEntries();
    ...
    // properly
    public static double getThresholdDependingOnRating(CollateralConfig marginCall,
                                                       MarginCallCreditRating mccCreditRating, int ratingsConfigId, JDate processDate, JDate valueDate) {

        if (mccCreditRating != null) {
            // check threshold type
            if (mccCreditRating.getThresholdType().equals(CollateralConfig.AMOUNT)) {
                return ELBEandKGRutilities.getThresholdAmountFromMccMatrix(ratingsConfigId, mccCreditRating,
            ...
            // {
            // return
            // ELBEandKGRutilities.getThresholdMCPercentFromMccMatrix(marginCall,
            // mccCreditRating, processDate);
            // }
            if (mccCreditRating.getThresholdType().equals(CollateralConfig.PERCENT)) {
                return ELBEandKGRutilities.getThresholdPercentFromMccMatrix(marginCall, mccCreditRating, processDate);
            }
            if (mccCreditRating.getThresholdType().equals(CollateralConfig.BOTH)) {
                double thresholdAmount = ELBEandKGRutilities.getThresholdAmountFromMccMatrix(ratingsConfigId,
                        mccCreditRating, marginCall.getCurrency(), valueDate);
                double thresholdPercent = ELBEandKGRutilities.getThresholdPercentFromMccMatrix(marginCall,
                        mccCreditRating, processDate);
    ...
    public static double getThresholdDependingOnAmount(CollateralConfig marginCall, String legalEntity, JDate date) {
        ...
        return thresholdAmount * CollateralUtilities.getFXRate(date, thresholdCcy, marginCall.getCurrency());
    ...
    public static double getThresholdDependingOnMcPercent(CollateralConfig marginCall, String legalEntity, JDate date) {
        ...
        double totalExposure = ELBEandKGRutilities.getMccTotalExposure(marginCall, date);
    ...
    public static double getThresholdDependingOnPercent(CollateralConfig marginCall, String legalEntity, JDate date) {
        ...
        double totalNominal = ELBEandKGRutilities.getMccTotalNominal(marginCall, date);
    ...
    public static double getThresholdDependingOnBoth(CollateralConfig marginCall, String legalEntity, JDate processDate,
                                                     JDate valueDate) {
        ...
        if (legalEntity.equals("PO")) { // ProcessingOrg
            thresholdAmount = marginCall.getPoNewThresholdAmount() * CollateralUtilities.getFXRate(valueDate,
                    marginCall.getPoNewThresholdCurrency(), marginCall.getCurrency());
            percentage = marginCall.getPoNewThresholdPercentage();
        } else { // Counterparty
            thresholdAmount = marginCall.getLeNewThresholdAmount() * CollateralUtilities.getFXRate(valueDate,
                    marginCall.getLeNewThresholdCurrency(), marginCall.getCurrency());
            percentage = marginCall.getLeNewThresholdPercentage();
        }

        double totalNominal = ELBEandKGRutilities.getMccTotalNominal(marginCall, processDate);
    ...
    public static double getThresholdAmountFromMccMatrix(int ratingsConfigId, MarginCallCreditRating mccCreditRating,
                                                         String baseCcy, JDate date) {

        MarginCallCreditRatingConfiguration mcRatingConfiguration;
        try {
            mcRatingConfiguration = CollateralUtilities.getMCRatingConfiguration(ratingsConfigId);
            if (mcRatingConfiguration != null) {
                double thresholdValue = new Amount(CollateralUtil.repairNumber(mccCreditRating.getThreshold()), 2).get();
                String thresholdCcy = mcRatingConfiguration.getThresholdCurrency();
                return thresholdValue * CollateralUtilities.getFXRate(date, thresholdCcy, baseCcy);
            }
        } catch (Exception e) {
            Log.error(ELBEandKGRutilities.class, e); //sonar
        }
    ...
    public static double getThresholdMCPercentFromMccMatrix(final CollateralConfig marginCall,
    ...
    public static double getThresholdPercentFromMccMatrix(final CollateralConfig marginCall,
                                                          final MarginCallCreditRating mccCreditRating, JDate date) {
    ...
    // type and get IA
    // properly
    public static double getIndAmountDependingOnRating(CollateralConfig marginCall,
                                                       MarginCallCreditRating mccCreditRating, int ratingsConfigId, JDate processDate, JDate valueDate) {

        if (mccCreditRating != null) {
            // check IA type
            if (mccCreditRating.getIaType().equals(CollateralConfig.AMOUNT)) {
                return getIaAmountFromMccMatrix(ratingsConfigId, mccCreditRating, marginCall.getCurrency(), valueDate);
            }
            if (mccCreditRating.getIaType().equals("MC_PERCENT")) {
                return getIaMCPercentFromMccMatrix(marginCall, mccCreditRating, processDate);
            }
            if (mccCreditRating.getIaType().equals("PERCENT")) {
                return getIaPercentFromMccMatrix(marginCall, mccCreditRating, processDate);
            }
            if (mccCreditRating.getIaType().equals("BOTH")) {
                double IaAmount = getIaAmountFromMccMatrix(ratingsConfigId, mccCreditRating, marginCall.getCurrency(),
                        valueDate);
                double IaPercent = getIaPercentFromMccMatrix(marginCall, mccCreditRating, processDate);
    ...
    public static double getIaAmountFromMccMatrix(int ratingsConfigId, MarginCallCreditRating mccCreditRating,
                                                  String baseCcy, JDate date) {
        MarginCallCreditRatingConfiguration mcRatingConfiguration;
        try {
            mcRatingConfiguration = CollateralUtilities.getMCRatingConfiguration(ratingsConfigId);
            if (mcRatingConfiguration != null) {
                //AAP FIX 07/06/2016
                double iaValue = new Amount(CollateralUtil.repairNumber(mccCreditRating.getIndependentAmount()), 2).get();
                //Delete this
                //double iaValue = Double.parseDouble(mccCreditRating.getIndependentAmount());
                String iaCcy = mcRatingConfiguration.getIaCurrency();
                return iaValue * CollateralUtilities.getFXRate(date, iaCcy, baseCcy);
            }
        } catch (Exception e) {
            Log.error(ELBEandKGRutilities.class, e); //sonar
        }
    ...
    public static double getIaMCPercentFromMccMatrix(final CollateralConfig marginCall,
                                                     final MarginCallCreditRating mccCreditRating, JDate date) {
    ...
    public static double getIaPercentFromMccMatrix(final CollateralConfig marginCall,
                                                   final MarginCallCreditRating mccCreditRating, JDate date) {
    ...
    public static Double getContractIA(final CollateralConfig marginCall) {
        if (Util.isEmpty(marginCall.getAdditionalField(IA_AMOUNT))) {
            return new Amount(marginCall.getAdditionalField(IA_AMOUNT), 2).get();
        } else {
            Double amount;
            String field = "";
            // GSM: 07/10/14. Fix to avoid format exception and log the contract
            // that failed
            try {
                field = marginCall.getAdditionalField(IA_AMOUNT);
                amount = Double.parseDouble(field);
                // new BigDecimal(amount).setScale(2, RoundingMode.DOWN);
                return (new BigDecimal(amount).setScale(2, RoundingMode.DOWN)).doubleValue();

            } catch (NumberFormatException e) {
                Log.error(ELBEandKGRutilities.class,
                        "NumberFormatException  reading field " + field + " for the contract " + marginCall.getName());
                // throw e;
            }
    ...
    // properly
    public static double getMtaDependingOnRating(CollateralConfig marginCall, MarginCallCreditRating mccCreditRating,
                                                 int ratingsConfigId, JDate processDate, JDate valueDate) {

        if (mccCreditRating != null) {
            // check threshold type
            if (mccCreditRating.getMtaType().equals(CollateralConfig.AMOUNT)) {
                return ELBEandKGRutilities.getMtaAmountFromMccMatrix(ratingsConfigId, mccCreditRating,
                        marginCall.getCurrency(), valueDate);
            }
            // MIG_V14
            // if
            ...
            // {
            // return
            // ELBEandKGRutilities.getMtaMCPercentFromMccMatrix(marginCall,
            // mccCreditRating, processDate);
            // }
            if (mccCreditRating.getMtaType().equals(CollateralConfig.PERCENT)) {
                return ELBEandKGRutilities.getMtaPercentFromMccMatrix(marginCall, mccCreditRating, processDate);
            }
            if (mccCreditRating.getMtaType().equals(CollateralConfig.BOTH)) {
                double mtaAmount = ELBEandKGRutilities.getMtaAmountFromMccMatrix(ratingsConfigId, mccCreditRating,
                        marginCall.getCurrency(), valueDate);
                double mtaPercent = ELBEandKGRutilities.getMtaPercentFromMccMatrix(marginCall, mccCreditRating,
                        processDate);
    ...
    // of contract
    public static double getMtaDependingOnAmount(CollateralConfig marginCall, String legalEntity, JDate date) {
        ...
        return mtaAmount * CollateralUtilities.getFXRate(date, mtaCcy, marginCall.getCurrency());

    }

    // Get MTA depending on mc_percent: get MTA mc_percent value specified on
    // legalEntity side of contract
    public static double getMtaDependingOnMcPercent(CollateralConfig marginCall, String legalEntity, JDate date) {
    ...
    // legalEntity side of contract
    public static double getMtaDependingOnPercent(CollateralConfig marginCall, String legalEntity, JDate date) {
    ...
    // them, and return lower one
    public static double getMtaDependingOnBoth(CollateralConfig marginCall, String legalEntity, JDate processDate,
                                               JDate valueDate) {
        ...
        if (legalEntity.equals("PO")) { // ProcessingOrg
            mtaAmount = marginCall.getPoMTAAmount()
                    * CollateralUtilities.getFXRate(valueDate, marginCall.getPoMTACurrency(), marginCall.getCurrency());
            percentage = marginCall.getPoMTAPercentage();
        } else { // Counterparty
            mtaAmount = marginCall.getLeMTAAmount()
                    * CollateralUtilities.getFXRate(valueDate, marginCall.getLeMTACurrency(), marginCall.getCurrency());
            percentage = marginCall.getLeMTAPercentage();
        }
    ...
    public static double getMtaAmountFromMccMatrix(int ratingsConfigId, MarginCallCreditRating mccCreditRating,
                                                   String baseCcy, JDate date) {
        ...
        try {
            mcRatingConfiguration = CollateralUtilities.getMCRatingConfiguration(ratingsConfigId);
            if (mcRatingConfiguration != null) {
                double mtaValue = new Amount(CollateralUtil.repairNumber(mccCreditRating.getMta()), 2).get();
                String mtaCcy = mcRatingConfiguration.getMtaCurrency();
                return mtaValue * CollateralUtilities.getFXRate(date, mtaCcy, baseCcy);
            }
        } catch (Exception e) {
            Log.error(ELBEandKGRutilities.class, e); //sonar
        }
    ...
    public static double getMtaMCPercentFromMccMatrix(final CollateralConfig marginCall,
                                                      final MarginCallCreditRating mccCreditRating, JDate date) {
    ...
    public static double getMtaPercentFromMccMatrix(final CollateralConfig marginCall,
                                                    final MarginCallCreditRating mccCreditRating, JDate date) {
