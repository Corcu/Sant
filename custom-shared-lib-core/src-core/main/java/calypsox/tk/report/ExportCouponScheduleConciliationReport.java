...
public class ExportCouponScheduleConciliationReport extends BondReport {

    public static final String EXPORT_COUPONSCHEDULECONCILIATION_REPORT = "ExportCouponScheduleConciliationReport";
        ...
        try {
            ...
            if (vBonds != null) {
                ...
                for (int i = 0; i < vBonds.size(); i++) {
                    if ((vBonds.get(i).getCouponSchedule().size() > 0)) {
                        final Vector<ExportCouponScheduleConciliationItem> exp_CouponScheduleConciliationItem = ExportCouponScheduleConciliationLogic
                                .getReportRows(vBonds.get(i), reportTemp.getValDate().toString(), dsConn, errorMsgsP);
                        for (int j = 0; j < exp_CouponScheduleConciliationItem.size(); j++) {
                            final ReportRow repRow = new ReportRow(vBonds.get(i), "Product");
                            repRow.setProperty("ExportCouponScheduleConciliationItem",
                                    exp_CouponScheduleConciliationItem.get(j));
            ...
            Log.error(this, "ExportCouponScheduleConciliationReport - " + e.getMessage());
