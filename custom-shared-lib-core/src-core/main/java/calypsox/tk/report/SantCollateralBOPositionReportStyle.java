 ...
 * 
 */
public class SantCollateralBOPositionReportStyle extends BOSecurityPositionReportStyle {
	...
	public static final String[] DEFAULT_COLUMN_NAMES = new String[] { SantCollateralBOPositionReportStyle.PO,
			COUNTERPARTY, CTR_NAME, BOSecurityPositionReportStyle.MCC_CONTRACT_TYPE, CTR_BASE_CCY, ELIGIBLE_COL,
			POS_STATUS };

	private final HashMap<String, PricingEnv> pricingEnvsCache = new HashMap<String, PricingEnv>();
		...
		if (pos instanceof InventoryCashPosition) {
			...
			if (ELIGIBLE_COL.equals(columnName)) {
				...
				if (columnDate != null) {
					...
					HashMap positions = (HashMap) row.getProperty(BOPositionReport.POSITIONS);
					if (positions == null) {
						return null;
					}
	 ...
	 * 
	 * @param reportDate
	 *            The date on which the report is launched
	 *            The settlement date of the collateral position
