...
public class ExportBondDirtyCleanPricesReport extends BondReport {
	...
	public ReportOutput load(@SuppressWarnings("rawtypes") final Vector errorMsgsP) {
		...
		if (null == attributes.get(FEED_NAME)) {
			Log.error(this, "ExportBondDirtyCleanPrices - FEED field is blank");
		...
		try {
			...
			if (vQuotes != null) {
				for (int i = 0; i < vQuotes.size(); i++) {
					final Vector<ExportBondDirtyCleanPricesItem> bondDirtyCleanPricesItem = ExportBondDirtyCleanPricesLogic
							.getReportRows(vQuotes.get(i), jdate.toString(), feed, price_type, dsConn, errorMsgsP);
					for (int j = 0; j < bondDirtyCleanPricesItem.size(); j++) {
						final ReportRow repRow = new ReportRow(bondDirtyCleanPricesItem.get(j));
			...
			Log.error(this, "ExportBondDirtyCleanPrices - " + e.getMessage());
