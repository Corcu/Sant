...
import calypsox.tk.report.SantThirdPartyInventoryViewReportStyle;
...
public class ThirdPartyPositionPropertiesTableBuilder {
	...
	public ThirdPartyPositionPropertiesTableBuilder() {
		initProps();
		display();
	}
	...
	private void initProps() {
		...
		this.positionDateProp = PropertyFactory.makeEnumProperty("Position Date",
				"Position Date", null, getMapFromList(getPositionDates()));
		this.positionClassProp = PropertyFactory.makeEnumProperty("Position Class",
				"Position Class", null, getMapFromList(getPositionClasses()));
		this.positionTypeProp = makePositiontTypeProperty();
		this.positionAggregationProp = PropertyFactory.makeEnumProperty("Aggregation",
				"Aggregation", null, getMapFromList(getAggregations()));
	...
	public ReportTemplate getTemplate(ReportTemplate template) {
		...
		if (valueObject != null) {
			...
			if (!Util.isEmpty(value)) {
				template.put(BOSecurityPositionReportTemplate.POSITION_VALUE,
						value);
			}
		}
		...
		if (valueObject != null) {
			...
			if (valueObject.getName().equals(
					SantThirdPartyInventoryViewReportStyle.NOMINAL)) {
				template.put(BOPositionReportTemplate.POSITION_VALUE,
						SantThirdPartyInventoryViewReportStyle.NOMINAL);
			} else {
				template.put(BOPositionReportTemplate.POSITION_VALUE,
						"Quantity");
