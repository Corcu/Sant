...
/**
 * Set the allocation's attribute AllocationMode to Margin
 * 
 * @author aela
 * 
 */
public class SantSetAllocAsMarginCollateralRule extends BaseCollateralWorkflowRule {

	@Override
	public String getDescription() {
		return "Set the allocation's attribute AllocationMode to Margin.";
	}
	...
	protected boolean isApplicable(TaskWorkflowConfig paramTaskWorkflowConfig, MarginCallEntry paramMarginCallEntry,
		...
		if ((allocs != null) && (allocs.size() > 0)) {
			for (MarginCallAllocation alloc : allocs) {
				alloc.addAttribute("AllocationMode", "Margin");
			}
		}
