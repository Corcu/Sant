...
public class EMAILGatewayNOTIFICATIONDocumentSender extends EMAILDocumentSender {
    ...
    public boolean send(final DSConnection dsCon, final SenderConfig senderConfig,
        ...
        try {
            ...
            if (po == null) {
                paramVector2.add("EmailGatewayNOTIFICATIONDocumentSender Unable to get sender information for message" + message.getLongId());
                return false;
            }
            if (cpty == null) {
                paramVector2.add("EmailGatewayNOTIFICATIONDocumentSender Unable to get reciever information for message" + message.getLongId());
                return false;
            }

        } catch (final Exception e) {
            Log.error(this, e);
            paramVector2.add("EmailGatewayNOTIFICATIONDocumentSender Unable to get the margin call detail for message " + message.getLongId());
            return false;
        }
        ...
        if (Util.isEmpty(emailData.getBody())) {
            try {
                ...
                Log.error(this, e);
                paramVector2.add("EmailGatewayNOTIFICATIONDocumentSender Unable to get email text for message " + message.getLongId());
                return false;
            }
        } else {
    ...
    private void saveAndResendMessage(EmailDataBuilder emailData, EmailMessage email, BOMessage message, String engineName, final Vector paramVector2, final boolean[] saved, DSConnection dsCon) {
        ...
        try {
            ...
            try {
                saveMessage(message, engineName, sendAgain, "Updated by EmailGatewayNOTIFICATIONDocumentSender");
            } catch (Exception e1) {
            ...
            try {
                saveMessage(message, engineName, errorSend, "Updated by EmailGatewayNOTIFICATIONDocumentSender");
            } catch (Exception e1) {
    ...
    private void saveAndSendMessage(EmailMessage email, BOMessage message, String engineName, final Vector paramVector2) {
        try {
            ...
            // apply the send action on the message
            try {
                saveMessage(message, engineName, Action.S_SEND, "Updated by EmailGatewayNOTIFICATIONDocumentSender");
...
//				saved[0] = true;
            ...
            Log.error(this, me);
            paramVector2.add("EmailGatewayNOTIFICATIONDocumentSender Error " + me.getMessage() + " " + Util.exceptionToString(me));
            try {
                saveMessage(message, engineName, errorSend, "Updated by EmailGatewayNOTIFICATIONDocumentSender");
            } catch (Exception exc) {
