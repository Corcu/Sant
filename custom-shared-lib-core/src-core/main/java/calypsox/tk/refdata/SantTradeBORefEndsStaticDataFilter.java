...
import static calypsox.util.TradeInterfaceUtils.TRD_IMP_FIELD_BO_REFERENCE;
 ...
 * check if the BO REF ends with the word "SWAP".
 * 
 * @author Guillermo Solano
 * @version 1.1; 12/07/2013
 * 
 */
@SuppressWarnings("deprecation")
public class SantTradeBORefEndsStaticDataFilter implements StaticDataFilterInterface {

	// Cte to define de SD filter name and Domain
	public final static String BO_REFERENCE_ENDS_WITH = "BORefEndsWithWord";
	...
	public boolean fillTreeList(DSConnection con, TreeList tl) {
		...
		nodes.add("SantTrade");
		tl.add(nodes, BO_REFERENCE_ENDS_WITH);
		return false;
	}
	...
	public Vector getTypeDomain(String attributeName) {
		...
		if (attributeName.equals(BO_REFERENCE_ENDS_WITH)) {
	...
	public Object getValue(Trade trade, LegalEntity le, String role, Product product, BOTransfer transfer,
		...
		final String boRef = trade.getKeywordValue(TRD_IMP_FIELD_BO_REFERENCE).trim();
		final String compare = ((String) element.getValues().get(0)).trim();

		if (Util.isEmpty(boRef) || Util.isEmpty(compare)) {
			return NOT_FOUND;
		}

		// satisfies the condition
		if (boRef.endsWith(compare)) {
			return compare;
		}
