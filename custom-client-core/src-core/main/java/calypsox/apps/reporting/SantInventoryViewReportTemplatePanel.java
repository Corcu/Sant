...
import calypsox.tk.report.inventoryview.DatePropertiesTableBuilder;
...
import calypsox.tk.report.inventoryview.PositionPropertiesTableBuilder;
import calypsox.tk.report.thirdpartyinventoryview.ProductPropertiesTableBuilder;

public class SantInventoryViewReportTemplatePanel extends ReportTemplatePanel {

	private static final long serialVersionUID = -2216340651361984998L;

	protected ReportTemplate template = null;

	private final PositionPropertiesTableBuilder positionBuilder = new PositionPropertiesTableBuilder();
	...
	public SantInventoryViewReportTemplatePanel() {
		init();
	}

	private void init() {

		JComponent dateComponent = this.dateBuilder.getComponent(new Dimension(250, 20));
		JComponent legalComponent = this.legalBuilder.getComponent(new Dimension(250, 90));

		JComponent positionComponent = this.positionBuilder.getComponent(new Dimension(250, 110));
		JComponent haircutComponent = this.haircutBuilder.getComponent(new Dimension(250, 60));
		JComponent crComponent = this.crBuilder.getComponent(new Dimension(350, 110));

		JComponent productComponent = this.productBuilder.getComponent(new Dimension(250, 50));
